...
export class Visuals extends Component {
  ...
  onTrackChange = async (track) => {
    ...
    const lookAtSegment = lookAt('loudness_max', data => console.log('Segment', data));
  ...
  render() {
    ...
    >
      ...
      <div className="background">
        <Surface width={400} height={400}>
          <Motion defaultStyle={{factor: 0, thickness: 0, loudness: 0, spacing: 0.2}} style={{
            factor: spring(promote ? 0 : 1, {stiffness: 70, damping: 5}),
            loudness: spring(loudness),
            thickness: spring(Math.max(0.1, pulse * loudness)),
            spacing: spring(loudness ** 2 + 0.4, {stiffness: 30, damping: 60, precision: 0.0001})
          }}>
            {({factor, thickness, loudness, spacing}) => {
              console.log('Loudness', loudness);
              return <Blur passes={2} factor={factor * 3}>
                <Pontus
                  thickness={thickness}
